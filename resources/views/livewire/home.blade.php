<div class="overflow-x-hidden bg-[#0B0F19] text-white selection:bg-cyan-500 selection:text-white" x-data="{ scroll: 0 }"
        ...
        <div class="container mx-auto px-6 relative z-10 text-center">
            ...
            <h1 class="text-6xl md:text-9xl font-black mb-6">
                <div class="glitch-wrapper">
                    <span class="glitch" data-text="NEXT LEVEL">NEXT LEVEL</span>
                </div>
                <br>
                DIGITAL.
            </h1>
            ...
            <div data-aos="fade-up" data-aos-delay="400"
                ...
                <a @mouseenter="$store.sfx.playHover()" href="#portfolio"
                    class="relative px-8 py-4 bg-white text-black font-bold text-lg rounded-full overflow-hidden group">
                    <span
                    ...
                    <span class="relative z-10 group-hover:text-white transition">Lihat Karya Kami</span>
                </a>
            </div>
        </div>
    </section>

    {{-- MARQUEE SECTION (SAMA SEPERTI SEBELUMNYA) --}}
    <section class="py-10 border-y border-white/5 bg-black/20 backdrop-blur-sm relative overflow-hidden">
        <div class="absolute inset-y-0 left-0 w-32 bg-gradient-to-r from-[#0B0F19] to-transparent z-20"></div>
        <div class="absolute inset-y-0 right-0 w-32 bg-gradient-to-l from-[#0B0F19] to-transparent z-20"></div>
        <div
            ...
            @foreach (range(1, 2) as $loop)
                <span class="text-2xl font-bold flex items-center gap-2 text-white"><svg class="w-8 h-8"
                        viewBox="0 0 24 24" fill="currentColor">
                        <path
                            d="M12 2L2 7l10 5 10-5-10-5zm0 9l2.5-1.25L12 8.5l-2.5 1.25L12 11zm0 2.5l-5-2.5-5 2.5L12 22l10-8.5-5-2.5-5 2.5z" />
                    </svg> Laravel</span>
                <span class="text-2xl font-bold flex items-center gap-2 text-white">⚡ Livewire</span>
                <span class="text-2xl font-bold flex items-center gap-2 text-white">🎨 Tailwind</span>
                <span class="text-2xl font-bold flex items-center gap-2 text-white">⚛️ React</span>
                <span class="text-2xl font-bold flex items-center gap-2 text-white">📱 Flutter</span>
                <span class="text-2xl font-bold flex items-center gap-2 text-white">☁️ AWS</span>
            @endforeach
        </div>
    </section>

    {{-- BENTO GRID (ABOUT) - DISEDERHANAKAN AGAR LEBIH CEPAT LOAD --}}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                <div data-aos="fade-right">
                    ...
                    <p class="text-slate-400 text-lg leading-relaxed mb-8">
                        ...
                        untuk bisnis Anda. Efisiensi, Skabilitas, dan Estetika adalah DNA kami.
                    </p>
                ...
                <div data-aos="fade-left" class="relative group">
                    <div
                        class="absolute -inset-1 bg-gradient-to-r from-cyan-600 to-purple-600 rounded-2xl blur opacity-25 group-hover:opacity-75 transition duration-1000 group-hover:duration-200">
                    </div>
                    @if ($page->getFirstMediaUrl('hero'))
                        <img src="{{ $page->getFirstMediaUrl('hero') }}"
                            class="relative rounded-2xl shadow-2xl w-full object-cover h-96 grayscale group-hover:grayscale-0 transition duration-500"
                            alt="About Us">
                    @else
                        <div
                            class="relative rounded-2xl shadow-2xl w-full h-96 bg-slate-800 flex items-center justify-center">
                            Image Not Found</div>
                    @endif
                </div>
            </div>
    ...
    {{-- ========================================== --}}
    {{-- BAGIAN 4: LAYANAN (SPOTLIGHT EFFECT)       --}}
    {{-- ========================================== --}}
    <section id="services" class="py-24 relative bg-[#0B0F19]">
        <div class="container mx-auto px-6 relative z-10">
            <div class="mb-20">
                <span class="text-cyan-400 font-bold tracking-widest uppercase text-sm mb-2 block">Our Expertise</span>
                <h2 class="text-5xl md:text-6xl font-black text-white">LAYANAN <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-600">PREMIUM</span>
                </h2>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 group/list">
                @foreach ($services as $service)
                    <div class="group relative h-full rounded-3xl bg-slate-900 border border-white/5 p-2 transition-all hover:border-white/10"
                        x-data="{ x: 0, y: 0 }"
                        ...
                        <div class="pointer-events-none absolute -inset-px rounded-3xl opacity-0 transition duration-300 group-hover:opacity-100"
                            :style="`background: radial-gradient(600px circle at ${x}px ${y}px, rgba(6,182,212,0.15), transparent 40%)`">
                        </div>

                        <div
                            class="relative h-full overflow-hidden rounded-2xl bg-[#0F1623] p-8 md:p-10 flex flex-col justify-between">
                            <div
                                class="absolute inset-0 z-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:16px_16px] opacity-20">
                            </div>

                            <div class="relative z-10">
                                <div
                                    class="mb-6 inline-flex items-center justify-center w-14 h-14 rounded-xl bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 shadow-[0_0_15px_rgba(6,182,212,0.1)] group-hover:bg-cyan-500 group-hover:text-white transition-colors duration-300">
                                    @svg($service->icon, 'w-7 h-7')
                                </div>

                                <h3
                                    class="text-2xl font-bold text-white mb-4 group-hover:text-cyan-400 transition-colors">
                                    {{ $service->title }}</h3>
                            ...
                            <div
                                class="relative z-10 mt-8 flex items-center gap-2 text-sm font-bold text-white opacity-0 transform translate-y-4 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">
            ...
            <div class="text-center mt-16">
                <a href="{{ route('services.index') }}" wire:navigate
                    ...
                    Lihat Seluruh Layanan <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
